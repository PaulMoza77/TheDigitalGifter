...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "openGift") {
      ...
      const { data: consumed, error: consumeErr } = await service.rpc("consume_christmas_gift_tree_open", {
        p_account_id: account.id,
        p_kind: kind,
      });
      if (consumeErr) throw consumeErr;
      const consumeRow = Array.isArray(consumed) ? consumed[0] : consumed;
      if (!consumeRow || consumeRow.ok !== true) {
        return jsonResponse({ error: kind === "free" ? "need_pack" : "no_opens" }, kind === "free" ? 402 : 409);
      }
      ...
      if (openErr) {
        if (openErr.code === "23505" && kind === "free") {
          // Refund the consumed free open if a concurrent free insert won.
          await service
            .from("christmas_gift_tree_accounts")
            .update({ free_opens_used: Math.max(0, Number(consumeRow.free_opens_used || 1) - 1) })
            .eq("id", account.id)
            .eq("free_opens_used", consumeRow.free_opens_used);
          return jsonResponse({
            ok: true,
            already: true,
            open_kind: "free",
      ...
      return jsonResponse({
        ...
        open_kind: kind,
        reward: publicReward,
        remaining_free_opens: opensOn ? remainingFreeOpens(Number(consumeRow.free_opens_used)) : 0,
        remaining_paid_opens: Number(consumeRow.paid_opens_remaining),
      });
    }
    ...
    if (action === "claimGuestAccount") {
      ...
      if (String(userAccount.id) === String(guestAccount.data.id)) {
      ...
      await service
        .from("christmas_gift_tree_opens")
        .update({ user_id: user.id, guest_token_hash: null, account_id: userAccount.id })
