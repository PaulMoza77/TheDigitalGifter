...
export function resolveServerOwnedOffer(offer: {
  ...
  active?: unknown;
} | null): { ok: true; amountCents: number; currency: "usd"; sku: typeof PET_SKU } | { ok: false; message: string } {
  ...
  if (sku !== PET_SKU) return { ok: false, message: "Unknown SKU." };
  ...
  return { ok: true, amountCents: Math.round(amount), currency: PET_CURRENCY, sku: PET_SKU };
}
