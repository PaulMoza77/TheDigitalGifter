...
export async function grantPaidGiftTreeOpens(input: {
  ...
  const { error: balErr } = await input.service
    .from("christmas_gift_tree_accounts")
    .update({
      paid_opens_remaining: Number(account.paid_opens_remaining || 0) + opens,
      paid_opens_granted: Number(account.paid_opens_granted || 0) + opens,
    })
    .eq("id", account.id);
