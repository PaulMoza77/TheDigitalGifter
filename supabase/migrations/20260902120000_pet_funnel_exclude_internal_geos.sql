-- Exclude founder geos (Romania / Italy) from pet funnel production KPIs.
-- Stores edge country + client IP on first-party events, marks internal geos as is_test,
-- backfills identifiable historical rows, and drops RO/IT from GA4 reporting aggregates.

begin;

create or replace function public.pet_funnel_country_is_internal(p_country text)
returns boolean
language sql
immutable
as $$
  select lower(btrim(coalesce(p_country, ''))) in (
    'ro', 'it', 'romania', 'italy'
  );
$$;

revoke all on function public.pet_funnel_country_is_internal(text) from public;
grant execute on function public.pet_funnel_country_is_internal(text) to anon, authenticated, service_role;
...
create index if not exists pet_funnel_events_country_created_idx
  on public.pet_funnel_events (country_code, created_at desc)
  where country_code is not null;

create index if not exists pet_v2_funnel_events_country_created_idx
  on public.pet_v2_funnel_events (country_code, created_at desc)
  where country_code is not null;

create index if not exists pet_v3_funnel_events_country_created_idx
  on public.pet_v3_funnel_events (country_code, created_at desc)
  where country_code is not null;

-- Historical: Romanian / Italian reverse-DNS already stored on V3.
update public.pet_v3_funnel_events e
set
  is_test = true,
  traffic_class = public.classify_pet_v3_traffic(
    true, e.utm_source, e.utm_medium, e.utm_campaign, e.campaign_id, e.adset_id, e.ad_id, e.creative_id,
    e.has_meta_click, e.fbc, e.fbp, e.referrer_host
  )
where coalesce(e.is_test, false) = false
  and (
    public.pet_funnel_country_is_internal(e.country_code)
    or lower(coalesce(e.client_ip_hostname, '')) like '%.ro'
    or lower(coalesce(e.client_ip_hostname, '')) like '%.it'
    or lower(coalesce(e.client_ip_hostname, '')) like '%rentalcarsoradea%'
    or lower(coalesce(e.client_ip_hostname, '')) like '%rdsnet%'
  );

-- Historical: obvious internal UTM markers on V1/V2 (no country stored yet).
update public.pet_funnel_events
set is_test = true
where coalesce(is_test, false) = false
  and (
    lower(coalesce(utm_source, '')) in ('internal', 'audit_smoke', 'ctrl_test')
    or lower(coalesce(utm_campaign, '')) in ('cat-v3-live-smoke', 'checkout-proof')
  );

update public.pet_v2_funnel_events
set is_test = true
where coalesce(is_test, false) = false
  and (
    lower(coalesce(utm_source, '')) in ('internal', 'audit_smoke', 'ctrl_test')
    or lower(coalesce(utm_campaign, '')) in ('cat-v3-live-smoke', 'checkout-proof')
    or funnel_session_id in (
      'cccccccc-cccc-4ccc-8ccc-cccccccccccc'::uuid,
      'ffffffff-ffff-4fff-8fff-ffffffffffff'::uuid
    )
  );

-- Mark any rows that already have RO/IT country_code (future-proof / partial backfills).
update public.pet_funnel_events
set is_test = true
where coalesce(is_test, false) = false
  and public.pet_funnel_country_is_internal(country_code);
...
-- New optional geo args change the signature; drop prior overloads first.
...
revoke all on function public.record_pet_funnel_event(
  text, uuid, text, uuid, text, text, text, text, text, text, text, text, text, text, text, integer, boolean, text, boolean, text, uuid, text, text, text
...
begin
  ...
  -- p_is_test is server-computed (geo/IP/UTM/hostname); never accept raw client flags alone.
  ...
  insert into public.pet_v3_funnel_events (
    ...
    traffic_class, displayed_price_cents, stripe_checkout_session_id, country_code
  )
  values (
...
revoke all on function public.record_pet_v3_funnel_event(
  text, uuid, text, text, text, text, text, text, text, text, text, text, text, text, integer, boolean, text, uuid, boolean, text, text, text, text, text, text, text, integer, text, text
) from public, anon;
...
begin
    ...
    select
      ...
      (select count(*)::int from public.pet_funnel_event_failures
        where created_at >= p_from and created_at < p_to
          and coalesce(funnel_dataset, 'v1') = 'v1') as v1_failed_write_count,
      (select count(*)::int from public.pet_funnel_event_failures
        where created_at >= p_from and created_at < p_to
          and coalesce(funnel_dataset, 'v1') = 'v2') as v2_failed_write_count,
      (select count(*)::int from public.pet_funnel_event_failures
  ...
  ga4_range as (
    ...
    where g.metric_date >= (p_from at time zone 'UTC')::date
      ...
      and not public.pet_funnel_country_is_internal(g.country)
  ),
  ga4_totals as (
    select
