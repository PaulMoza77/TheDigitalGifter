...
-- Atomic consume: prevents double-spend of free/paid opens under concurrency.
create or replace function public.consume_christmas_gift_tree_open(
  p_account_id uuid,
  p_kind text
)
returns table (
  ok boolean,
  free_opens_used integer,
  paid_opens_remaining integer
)
language plpgsql
security definer
...
as $$
declare
  rec public.christmas_gift_tree_accounts%rowtype;
begin
  if p_kind = 'free' then
    update public.christmas_gift_tree_accounts
    set free_opens_used = free_opens_used + 1
    where id = p_account_id
      and free_opens_used < 1
    returning * into rec;
  elsif p_kind = 'paid' then
    update public.christmas_gift_tree_accounts
    set paid_opens_remaining = paid_opens_remaining - 1
    where id = p_account_id
      and paid_opens_remaining > 0
    returning * into rec;
  else
  ...
  return query select true, rec.free_opens_used, rec.paid_opens_remaining;
end;
$$;

revoke all on function public.consume_christmas_gift_tree_open(uuid, text) from public, anon, authenticated;
grant execute on function public.consume_christmas_gift_tree_open(uuid, text) to service_role;

-- Atomic grant increment so concurrent Stripe events cannot drop a pack credit.
create or replace function public.credit_christmas_gift_tree_opens(
  p_account_id uuid,
  p_opens integer
)
returns table (
...
declare
  rec public.christmas_gift_tree_accounts%rowtype;
begin
  if p_opens is null or p_opens < 1 then
    return query select false, 0, 0;
    return;
  end if;
  ...
  update public.christmas_gift_tree_accounts
  set
    paid_opens_remaining = paid_opens_remaining + p_opens,
    paid_opens_granted = paid_opens_granted + p_opens
  where id = p_account_id
  returning * into rec;
  ...
  return query select true, rec.paid_opens_remaining, rec.paid_opens_granted;
end;
$$;

revoke all on function public.credit_christmas_gift_tree_opens(uuid, integer) from public, anon, authenticated;
grant execute on function public.credit_christmas_gift_tree_opens(uuid, integer) to service_role;
