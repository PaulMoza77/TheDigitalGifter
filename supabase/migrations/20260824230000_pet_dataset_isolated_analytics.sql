-- Isolate V1/V2 first-party and Stripe by dataset/table, Meta by campaign_id.
-- Do not require campaign_id on first-party rows for the selected variant funnel.

begin;

alter table public.pet_funnel_event_failures
  add column if not exists funnel_dataset text;

do $$
begin
  if not exists (
    select 1 from pg_constraint where conname = 'pet_funnel_event_failures_dataset_chk'
  ) then
    alter table public.pet_funnel_event_failures
      add constraint pet_funnel_event_failures_dataset_chk
      check (funnel_dataset is null or funnel_dataset in ('v1', 'v2'));
  end if;
end
$$;

update public.pet_funnel_event_failures
set funnel_dataset = coalesce(funnel_dataset, 'v1')
where funnel_dataset is null;
