...
-- christmas_orders.locale already exists (default 'en'); tighten + persist from checkout.
-- Does not touch send-a-gift / gift-tree schemas.
-- Welcome journey is explicitly deferred (no Christmas-scoped lead capture).

update public.christmas_orders
set locale = case
  when lower(coalesce(locale, 'en')) like 'ro%' then 'ro'
  else 'en'
end
where locale is null
   or lower(locale) not in ('en', 'ro');

alter table public.christmas_orders
  drop constraint if exists christmas_orders_locale_chk;
alter table public.christmas_orders
  add constraint christmas_orders_locale_chk check (locale in ('en', 'ro'));
...
comment on table public.christmas_lifecycle_events is
  'Idempotent Christmas suite lifecycle email ledger. Marketing sends gated by env + email_preferences. Welcome deferred.';

comment on column public.christmas_orders.locale is
  'Persisted checkout locale (en|ro). Transactional emails use this, never webhook Accept-Language.';
