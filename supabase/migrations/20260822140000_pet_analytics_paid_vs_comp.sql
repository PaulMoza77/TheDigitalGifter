-- Classify pet orders for analytics: paid vs 100% promo comps vs Stripe test mode.
-- Does not delete orders. Does not change Meta/GA4 sync tables.

begin;

create or replace function public.pet_order_analytics_class(
  p_session_id text,
...
as $$
  select case
    when coalesce(p_session_id, '') like 'cs_test%'
      or coalesce(p_payment_intent_id, '') like 'pi_test%'
      then 'test'
    when coalesce(p_discount_percent, 0) >= 100
      or coalesce(p_session_id, '') like 'promo:%'
      or coalesce(p_payment_status, '') in ('no_payment_required', 'not_required')
      or coalesce(p_charged_amount_cents, p_amount_cents, 0) <= 0
      then 'free'
    else 'paid'
  end;
$$;

revoke all on function public.pet_order_analytics_class(text, text, integer, integer, integer, text) from public, anon;
grant execute on function public.pet_order_analytics_class(text, text, integer, integer, integer, text)
