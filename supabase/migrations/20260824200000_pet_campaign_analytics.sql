...
-- Adds funnel-variant mapping on the Meta allowlist and filters Meta + first-party
-- (V1 and V2) + attributed Stripe by campaign_id. Does not change public funnels,
-- prices, Stripe checkout, or Meta campaign objects.

begin;

alter table public.pet_meta_campaign_allowlist
  add column if not exists funnel_variant text,
  add column if not exists utm_campaign_aliases text[] not null default '{}'::text[],
  add column if not exists measurement_reliable_from timestamptz;

do $$
begin
  if not exists (
    select 1 from pg_constraint
    where conname = 'pet_meta_campaign_allowlist_variant_chk'
  ) then
    alter table public.pet_meta_campaign_allowlist
      add constraint pet_meta_campaign_allowlist_variant_chk
      check (funnel_variant is null or funnel_variant in ('v1', 'v2_preview'));
  end if;
end
$$;

comment on column public.pet_meta_campaign_allowlist.funnel_variant is
  'Explicit analytics funnel mapping. Never infer from campaign name.';
comment on column public.pet_meta_campaign_allowlist.utm_campaign_aliases is
  'Unique utm_campaign values that may resolve to this campaign_id when campaign_id is missing. Ambiguous aliases are ignored.';
comment on column public.pet_meta_campaign_allowlist.measurement_reliable_from is
  'First-party measurement for this campaign is reliable at/after this timestamp.';

update public.pet_meta_campaign_allowlist
set funnel_variant = coalesce(funnel_variant, 'v1'),
    measurement_reliable_from = coalesce(measurement_reliable_from, timestamptz '2026-08-24 00:00:00+00'),
    updated_at = now()
where campaign_id = '120253346791240170';

update public.pet_funnel_measurement_settings
set reliable_from = coalesce(reliable_from, timestamptz '2026-08-24 00:00:00+00')
where id = 1;

create index if not exists pet_funnel_events_campaign_created_idx
  on public.pet_funnel_events (campaign_id, created_at desc);

create index if not exists pet_v2_funnel_events_campaign_created_idx
  on public.pet_v2_funnel_events (campaign_id, created_at desc);

create or replace function public.pet_analytics_resolve_campaign_id(
  p_event_campaign_id text,
  p_utm_campaign text
)
returns text
language plpgsql
...
begin
  cid := nullif(btrim(coalesce(p_event_campaign_id, '')), '');
  if cid is not null then
    return cid;
  end if;
  if nullif(btrim(coalesce(p_utm_campaign, '')), '') is null then
    return null;
  end if;

  select count(*)::int, min(a.campaign_id)
    into match_count, cid
  from public.pet_meta_campaign_allowlist a
  where a.enabled
    and exists (
      select 1
      from unnest(coalesce(a.utm_campaign_aliases, '{}'::text[])) as alias
      where lower(btrim(alias)) = lower(btrim(p_utm_campaign))
    );
...
revoke all on function public.pet_analytics_resolve_campaign_id(text, text) from public, anon;
grant execute on function public.pet_analytics_resolve_campaign_id(text, text) to authenticated, service_role;

create or replace function public.admin_upsert_pet_campaign_analytics_config(
  p_campaign_id text,
  p_funnel_variant text default null,
  p_display_name text default null,
  p_utm_campaign_aliases text[] default null,
...
begin
  ...
  if p_campaign_id is null or p_campaign_id !~ '^[0-9]{5,}$' then
    raise exception 'invalid campaign id' using errcode = '22023';
  end if;
  variant := nullif(btrim(coalesce(p_funnel_variant, '')), '');
  if variant is not null and variant not in ('v1', 'v2_preview') then
    raise exception 'invalid funnel variant' using errcode = '22023';
  end if;
  aliases := coalesce(p_utm_campaign_aliases, '{}'::text[]);

  insert into public.pet_meta_campaign_allowlist (
    campaign_id, label, enabled, funnel_variant, utm_campaign_aliases, measurement_reliable_from, updated_at
  )
  values (
    p_campaign_id,
  ...
  on conflict (campaign_id) do update set
    label = case
      when nullif(btrim(coalesce(p_display_name, '')), '') is not null then btrim(p_display_name)
      else public.pet_meta_campaign_allowlist.label
    end,
    funnel_variant = variant,
    utm_campaign_aliases = aliases,
    measurement_reliable_from = coalesce(p_measurement_reliable_from, public.pet_meta_campaign_allowlist.measurement_reliable_from),
    enabled = true,
    updated_at = now();
...
revoke all on function public.admin_upsert_pet_campaign_analytics_config(text, text, text, text[], timestamptz)
  from public, anon;
grant execute on function public.admin_upsert_pet_campaign_analytics_config(text, text, text, text[], timestamptz)
  to authenticated, service_role;

drop function if exists public.admin_pet_funnel_analytics(timestamptz, timestamptz, timestamptz, timestamptz);
...
begin
    ...
    where e.created_at >= p_from
      ...
      and (
        case
          ...
          when view_mode = 'campaign' then r.resolved_campaign_id = campaign_filter
          else true
        end
      )
  ...
  select jsonb_build_object(
    ...
    'tracking_health', jsonb_build_object(
      'first_party_landing_sessions', (select v1_landings + v2_landings from tracking_health),
      'v1_landing_sessions', (select v1_landings from tracking_health),
      'v2_landing_sessions', (select v2_landings from tracking_health),
      'latest_first_party_event_at', (select greatest(latest_v1_at, latest_v2_at) from tracking_health),
