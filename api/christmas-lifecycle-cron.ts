...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (lifecycleMarketingEnabled()) {
    const { data: livePackages } = await service
      .from("christmas_packages")
      .select("product_id, purchasable, active")
      .eq("purchasable", true)
      .eq("active", true);
    const productIds = [
      ...new Set(
        (livePackages || [])
          .map((row) => asString((row as { product_id?: string }).product_id))
          .filter(Boolean),
      ),
    ];
    ...
    if (productIds.length) {
      ...
      for (const product of products || []) {
        const key = asString((product as { product_key?: string }).product_key);
        if (key) live.add(key);
      }
    }
