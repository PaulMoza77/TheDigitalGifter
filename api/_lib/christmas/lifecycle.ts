...
export function lifecycleMarketingEnabled(): boolean {
  return asString(process.env.CHRISTMAS_LIFECYCLE_MARKETING_ENABLED).toLowerCase() === "true";
}
...
export async function claimAndSendLifecycleEmail(input: {
  ...
  const eventKey = lifecycleEventKey(input.template, input.order.id, input.eventSuffix);
  ...
  const { data: claimRaw } = await input.service.rpc("claim_christmas_lifecycle_event", {
    p_event_key: eventKey,
    p_template_key: input.template,
    p_category: category,
    p_order_id: input.order.id,
  ...
  const claim = claimRaw as { claimed?: boolean; status?: string } | null;
