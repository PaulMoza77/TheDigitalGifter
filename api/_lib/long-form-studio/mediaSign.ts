...
function secret(): string {
  return String(process.env.CLIP_FACTORY_SIGNING_SECRET || process.env.SUPABASE_SERVICE_ROLE_KEY || "tdg-long-form").slice(0, 80);
}
...
export function verifyLongFormSignature(id: string, exp: string | number, sig: string): boolean {
  ...
  const expected = signLongFormMedia(id, expires);
...
export function signedLongFormPath(kind: "video" | "thumb" | "audio", id: string, ttlSec = 60 * 60 * 12): string {
