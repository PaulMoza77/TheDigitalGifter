...
Accepted for `CHRISTMAS-027` (`tdg-christmas-wishlist-gift-finder-010`).
...
This product is **not** `/send-a-gift` and **not** `/christmas/gifts` (Gift Tree chance funnel owns that path). Canonical route is `/christmas/gift-finder`.

## Decision

1. **Primary provider:** OpenAI Chat Completions (`OPENAI_MODEL` or `gpt-4o-mini`), server-owned system prompt, JSON schema output.
2. **Fallback:** Deterministic curated idea catalog (`server_curated_v1`; `CHRISTMAS_GIFT_FINDER_MODE=curated` or automatic on quota/billing/network failure). Catalog lives in the service layer only — the browser never invents ideas.
...
7. **Affiliate outbound:** DEFERRED — use `search_query` + TDG preview CTAs (no Buy now / invented prices / merchant feeds).
8. **Persistence:** `christmas_gift_finder_sessions` + `christmas_gift_finder_results` enable Add-to-Wishlist without regenerating.
...
- `/christmas/gifts` stays free for the Gift Tree chance funnel.
