...
**Photo generator update (`tdg-christmas-photo-generator-003`):** `/christmas/photo-generator` is a real funnel with local blurred-original preview (zero pre-pay Replicate), post-pay `christmas-photo-generate`, and production purchase still disabled until package price is intentionally configured (GAP-007). See `docs/TDG_CHRISTMAS_PHOTO_GENERATOR.md`.
