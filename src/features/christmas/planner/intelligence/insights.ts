  ...
  if (missing.length > 0 && snapshot.recipients.length > 0) {
    ...
    out.push(
      insight({
        ...
        reason: "These people do not have a gift marked planned, ordered, or wrapped yet.",
  ...
  if (unordered.length > 0 && snapshot.daysLeft <= 28) {
    out.push(
      insight({
        ...
        reason: "These gifts are still an idea or a plan, so they have not been ordered.",
  ...
  for (const row of lateDeliveryGifts(snapshot)) {
    out.push(
      insight({
        ...
        reason: "The delivery date you saved is within three days of Christmas.",
  ...
  for (const row of giftsArrivingAfterTravel(snapshot)) {
    out.push(
      insight({
        ...
        reason: "This delivery date is after the day your trip starts.",
  ...
  for (const row of recipientBudgets.filter((r) => r.status === "over" && r.budgetMinor != null)) {
    out.push(
      insight({
        ...
        reason: "This uses that person's gift budget and the prices saved on their gifts.",
  ...
  if (budget.totalBudgetMinor != null && budget.overForecastMinor > 0) {
    out.push(
      insight({
        ...
        reason: "This adds gift prices and other planned costs once. A gift is not counted again as a separate expense.",
  ...
  if (budget.remainingAfterSpendMinor != null && budget.remainingAfterSpendMinor >= 0 && budget.totalBudgetMinor) {
    out.push(
      insight({
        ...
        reason: "This is your Christmas total minus money already spent.",
    ...
    if (used >= 78) {
      out.push(
        insight({
          ...
          reason: "Gift spending comes from the prices on gifts you have already bought.",
    ...
    if (row.budgetMinor > 0 && row.spentMinor > row.budgetMinor) {
      out.push(
        insight({
          ...
          reason: "This category is over the amount you set aside for it.",
  ...
  if (budget.giftsWithoutPriceCount > 0) {
    out.push(
      insight({
        ...
        reason: "These gifts do not have a price saved yet.",
    ...
    if (gap > budget.totalBudgetMinor * 0.4 && snapshot.recipients.length > 0) {
      out.push(
        insight({
          ...
          reason: "Part of your Christmas total is not tied to a gift or another planned cost yet.",
  ...
  for (const meal of detectFoodCompleteness(snapshot)) {
    ...
    out.push(
      insight({
        ...
        reason: "A full menu usually includes these courses. You can skip any of them.",
    ...
    if (gaps > 0) {
      out.push(
        insight({
          ...
          reason: "These guests are still marked maybe.",
    ...
    if (diet > 0) {
      out.push(
        insight({
          ...
          reason: "A saved dietary note does not appear in the current dish names.",
    ...
    if (homePrepOpen(snapshot) > 0 && snapshot.daysLeft <= 10) {
      out.push(
        insight({
          ...
          reason: "Hosting is on, and some home prep is still open.",
  ...
  if (missingHostingTemplateKeys(snapshot).length > 0) {
    out.push(
      insight({
        ...
        reason: "Hosting is on, and these hosting steps are not on your task list yet.",
  ...
  if (conflicts.length > 0 && depart) {
    out.push(
      insight({
        ...
        reason: "These open tasks are due after you leave, or they have no date yet.",
  ...
  if (openOverdue.length >= 3) {
    out.push(
      insight({
        ...
        reason: "These tasks are still open and their dates are before today.",
  ...
  if (cardsNeeded > 0 && snapshot.daysLeft <= 12) {
    out.push(
      insight({
        ...
        reason: "These cards are still marked as needed.",
