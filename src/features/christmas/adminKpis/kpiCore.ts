 ...
 * Task: Christmas founder KPI dashboard (admin /admin/christmas-kpis)
 *
 * Commercial truth = christmas_orders (payment/fulfillment).
 * Funnel stage counts = christmas_funnel_events (aggregate unique sessions, not cohort-exact).
 * Checkout session created ≠ purchase.
 */
...
export const CHRISTMAS_KPI_SOURCE_MATRIX = [
  ...
  {
    stage: "abandoned checkout",
    source: "christmas_orders with stripe_checkout_session_id and payment_status=pending",
    id: "christmas_orders.id",
    reliability: "derived",
  },
] as const;
...
export function buildChristmasKpiSnapshot(input: {
  ...
  for (const o of paid) {
    const c = String(o.currency || "eur").toLowerCase();
  ...
  const telemetryQuality: string[] = [
    ...
    "Checkout sessions created (stripe_checkout_session_id) are not purchases.",
...
export function isChristmasKpiEmpty(snap: ChristmasKpiSnapshot): boolean {
  return (
