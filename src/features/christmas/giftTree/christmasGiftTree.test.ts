...
describe("christmas gift tree chance math", () => {
  ...
  it("identity keys are scoped and grants are idempotent per order", () => {
    ...
    expect(GIFT_TREE_BOX_POSITIONS).toHaveLength(9);
  });
});
...
describe("christmas gift tree wiring", () => {
  ...
  it("migration is identity-scoped, additive, and consumes atomically", () => {
    ...
    expect(sql).toContain("consume_christmas_gift_tree_open");
    expect(sql).toContain("credit_christmas_gift_tree_opens");
  ...
  it("edge funnel and webhook grant server rewards only", () => {
    ...
    expect(fn).toContain("consume_christmas_gift_tree_open");
    expect(fn).not.toContain("client_reward");
    expect(fn).not.toContain("__test_force");
  ...
  it("page never hardcodes a fake win and wires Apple Pay checkout", () => {
    ...
    expect(page).toContain("checkoutLock");
