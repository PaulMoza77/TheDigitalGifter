...
export default function ChristmasGiftTreePage() {
  ...
  const packs = status?.packs?.length ? status.packs : PRODUCT?.packages.map((pkg) => ({
    ...
    >
      ...
      <div className="mx-auto max-w-lg px-4 pb-24 pt-8">
        ...
        <p className="mt-2 text-center text-sm text-rose-100/75">
          Tap a gift to open it. Outcomes come from the catalog — this page never invents a prize.
        </p>
        ...
        <div className="mt-6">
          <ChristmasTreeVisual style="classic" decorations={defaultDecorations()} />
        </div>
        ...
        <section className="mt-6" aria-label="Gifts on the chance tree">
          <h2 className="text-sm font-medium text-rose-100/90">Gifts on the tree</h2>
            ...
            {giftTreeBoxSlots(GIFT_TREE_BOX_COUNT).map((slot) => {
              ...
              return (
                <li key={slot}>
                  <button
                    type="button"
                    disabled={busy || opened || remaining <= 0}
                    aria-label={opened ? `Opened gift ${slot + 1}` : `Open gift ${slot + 1}`}
        ...
        <section className="mt-10 space-y-4" aria-label="Gift open packs">
          ...
          <ul className="space-y-3">
            {packs.map((pack) => (
              ...
              >
                ...
                >
                  {pack.purchasable ? `Buy ${pack.opens} open${pack.opens === 1 ? "" : "s"}` : "Pack not purchasable"}
                </button>
              </li>
            ))}
          </ul>
