...
import { normalizeChristmasLocale } from "../locale";
...
export function abandonedCheckoutEligibility(input: {
  ...
  const started = Date.parse(order.checkoutStartedAt || order.createdAt || "");
...
export function lifecycleEmailCopy(
  ...
  switch (template) {
    ...
    case "generation_started": {
      const subject = ro ? `Generăm ${product}…` : `Creating your ${product}…`;
      const body = ro
    ...
    case "generation_ready": {
      const subject = ro ? `${product} este gata` : `Your ${product} is ready`;
    ...
    case "cross_sell": {
      ...
      const unsub = vars.unsubscribeUrl || "/unsubscribe";
      const subject = ro ? `Poate îți place și: ${name}` : `You might also like: ${name}`;
      const body = ro
...
/** Welcome is explicitly deferred — no Christmas-scoped lead capture exists. */
export const CHRISTMAS_WELCOME_STATUS = {
  state: "deferred",
  reason: "blocked_by_lead_capture",
} as const;
