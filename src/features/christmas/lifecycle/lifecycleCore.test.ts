...
import {
  resolveCheckoutLocaleFromHints,
  normalizeChristmasLocale,
} from "../locale";
import {
  abandonedCheckoutEligibility,
  abandonedResumePath,
  CHRISTMAS_WELCOME_STATUS,
  crossSellTargets,
...
describe("christmas lifecycle core", () => {
  it("resolves persisted locale with EN fallback", () => {
    ...
    expect(resolvePersistedOrderLocale("RO-ro")).toBe("ro");
    expect(resolvePersistedOrderLocale(null)).toBe("en");
    expect(resolvePersistedOrderLocale("xx")).toBe("en");
    expect(normalizeChristmasLocale("ro-RO")).toBe("ro");
  });

  it("captures checkout locale from URL/html/navigator hints", () => {
    expect(resolveCheckoutLocaleFromHints(["ro"])).toBe("ro");
    expect(resolveCheckoutLocaleFromHints(["/christmas?locale=ro"])).toBe("ro");
    expect(resolveCheckoutLocaleFromHints(["en-US", "ro-RO"])).toBe("ro");
    expect(resolveCheckoutLocaleFromHints(["en-US", null])).toBe("en");
  });
  ...
  it("product landing paths are public routes", () => {
    expect(productLandingPath("christmas_santa_video")).toBe("/christmas/santa-video");
    ...
    expect(abandonedResumePath(order({ sourceRoute: null }))).toContain("photo-generator");
  });
  ...
  it("explicitly defers welcome until Christmas lead capture exists", () => {
    expect(CHRISTMAS_WELCOME_STATUS.state).toBe("deferred");
    expect(CHRISTMAS_WELCOME_STATUS.reason).toBe("blocked_by_lead_capture");
  });
});
