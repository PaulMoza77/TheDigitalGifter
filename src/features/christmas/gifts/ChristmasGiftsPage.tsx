  ...
  const onPurchase = useCallback(async (packageKey: string) => {
    ...
    try {
      ...
      const result = await startChristmasCheckout({
        ...
        currency: "usd",
        landing_path: "/christmas/tree-gifts",
        source_route: "/christmas/tree-gifts",
        guest_token: guestToken,
        success_url: `${window.location.origin}/christmas/tree-gifts?gift_chances=1&package=${encodeURIComponent(packageKey)}`,
        cancel_url: `${window.location.origin}/christmas/tree-gifts?gift_chances=cancel`,
      });
      setCheckout({
    ...
    <>
      {embedded ? null : (
        <PageHead
          ...
          url="https://www.thedigitalgifter.com/christmas/tree-gifts"
        />
      )}
...
export default function ChristmasGiftsPage() {
  return <ChristmasGiftsExperience />;
}
