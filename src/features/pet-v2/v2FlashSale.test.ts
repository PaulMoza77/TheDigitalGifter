...
describe("V2 rolling 24h sale", () => {
  it("always charges $0.99 with $27 compare-at", () => {
    const offer = v2FlashSale(PET_V2_SALE_EPOCH_MS + 1000);
    expect(offer.amountCents).toBe(99);
    expect(offer.priceDisplay).toBe("$0.99");
  ...
  it("resets the timer at each 24h boundary while price stays $0.99", () => {
