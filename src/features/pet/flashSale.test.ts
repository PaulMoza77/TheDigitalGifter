...
describe("pet $12 flash sale", () => {
  it("charges $12 until the real 24-hour expiry, then returns to $27", () => {
    ...
    expect(applyPetFlashSaleAmount(PET_PRICE_CENTS, PET_SALE_EXPIRES_AT_MS - 1)).toBe(1200);
  ...
  it("refreshes unpaid checkout after the sale expires so Stripe cannot keep $12", () => {
    expect(checkoutAmountNeedsRefresh(1200, 2700)).toBe(true);
    expect(checkoutAmountNeedsRefresh(1200, 1200)).toBe(false);
  });

  it("keeps $27 as the list price and overlays $12 on the live dog funnel", () => {
    expect(PET_PRICE_CENTS).toBe(2700);
    expect(PET_SALE_PRICE_CENTS).toBe(1200);
    ...
    expect(readSrc("supabase/functions/_shared/pet/flashSale.ts")).toContain("PET_SALE_PRICE_CENTS = 1200");
