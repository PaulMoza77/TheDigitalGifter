import { PET_V3_SESSION_KEY } from "./types";
...
function readId(storage: Storage | undefined): string | null {
  ...
  try {
    const value = storage.getItem(PET_V3_SESSION_KEY);
...
function writeId(storage: Storage | undefined, id: string) {
  if (!storage) return;
  try {
    storage.setItem(PET_V3_SESSION_KEY, id);
  } catch {
    /* private mode */
  }
}

/** Isolated from V1 and V2 funnel sessions. */
export function getPetV3SessionId(): string {
