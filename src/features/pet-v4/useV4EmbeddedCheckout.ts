...
import { v2PackOfferCopy } from "../pet-v2/V2PackOffer";
...
} from "../pet-v2/v2CheckoutHold";
import { getPetV4SessionId } from "./session";
import {
  PET_V2_PRICE_CENTS,
...
import { PET_V4_ORDER_VARIANT, PET_V4_PRICE_CENTS } from "./types";
...
function v4CancelUrl(species: PetV2Species, origin = typeof window !== "undefined" ? window.location.origin : ""): string {
  return `${origin}${petV4LandingPath(species)}?checkout=canceled`;
}
...
function applyCheckoutResult(input: {
  ...
  trackPetV4Event({
    eventName: "v4_checkout_session_created",
    species,
    amountCents,
...
export function useV4EmbeddedCheckout(input: {
  active: boolean;
  species: PetV2Species;
  ...
  const recoverExistingOrderCheckout = useCallback(async () => {
    ...
    const funnelSessionId = getPetV4SessionId();
    ...
    const checkout = await api.createStripeCheckout({
      ...
      cancelUrl: v4CancelUrl(input.species),
  ...
  const bootstrap = useCallback(async () => {
    ...
    try {
      ...
      const order = await api.createOrder({
        ...
        funnelVariant: PET_V4_ORDER_VARIANT,
      });
...
export { validateAndUpdateV2OrderContact, V2_CONTACT_UPDATE_ERROR } from "../pet-v2/v2ContactUpdate";
