...
describe("gift browse empty-state wiring", () => {
  ...
  it("defines ChristmasPortraitFunnelPage so the browse empty path can render", () => {
    const app = readSrc("src/App.tsx");

    expect(app).toMatch(
      /const ChristmasPortraitFunnelPage = lazy\(\s*\(\) => import\("@\/features\/christmas\/ChristmasPortraitFunnelPage"\)/,
    );
  });
});
