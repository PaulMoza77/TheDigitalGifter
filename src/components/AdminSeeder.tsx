...
export function AdminSeeder() {
  ...
  const addMoreTemplates = useAddMoreTemplatesMutation();
  const clearAllTemplates = useClearTemplatesMutation();

  const handleSeed = async () => {
    setIsSeeding(true);
    try {
      const result = await seedTemplates.mutateAsync();
      if (result.success) {
  ...
  const handleAddMore = async () => {
    ...
    try {
      const result = await addMoreTemplates.mutateAsync();
  ...
  const handleClear = async () => {
    ...
    setIsClearing(true);
    try {
      const result = await clearAllTemplates.mutateAsync();
    ...
    <div className="bg-white rounded-lg shadow-sm border p-4">
      ...
      <div className="space-y-3">
        ...
        <div className="text-xs text-gray-500 mt-2">
          <p>
            • <strong>Seed Initial:</strong> Adds the basic Christmas card
            templates
          </p>
          <p>
            • <strong>Add More:</strong> Adds additional premium templates
            (cabin, forest, etc.)
          </p>
          <p>
            • <strong>Clear All:</strong> Removes all templates from database
          </p>
        </div>
      </div>
    </div>
