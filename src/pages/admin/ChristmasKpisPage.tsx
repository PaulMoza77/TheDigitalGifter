...
import { loadChristmasKpiSources } from "@/features/christmas/adminKpis/kpiService";
...
export default function ChristmasKpisPage() {
  ...
  const load = useCallback(async () => {
    ...
    const result = await loadChristmasKpiSources();
    if (result.errors.length === 2) {
      setError(result.errors.join(" · ") || "Failed to load KPIs");
      setEvents([]);
      setOrders([]);
      setPartialErrors([]);
      setLoading(false);
    ...
    if (result.errors.length) {
      toast.message("Some KPI sources failed", { description: result.errors.join(" · ") });
    }
  }, []);
  ...
  const snap = useMemo(
    ...
    [preset, events, orders],
  );

  const empty = !loading && !error && isChristmasKpiEmpty(snap);
  const currency = snap.commercial.displayCurrency;
  ...
  return (
    <div className="space-y-6 p-6 text-slate-100">
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-slate-400">
            ...
            cohort-exact conversion. Checkout sessions are not purchases. No tokens, media, or
            customer emails.
          </p>
        </div>
        <Button
      ...
      {empty ? (
        <div
          data-testid="christmas-kpis-empty"
          className="rounded-xl border border-dashed border-slate-700 bg-slate-950/40 p-8 text-sm text-slate-400"
        >
      ...
      {!loading && !error ? (
        <>
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Stat
              label="Gross revenue"
              value={formatCents(snap.commercial.grossRevenueCents, currency)}
            />
            <Stat
              label="AOV"
              value={
                snap.commercial.aovCents == null
                  ? "—"
                  : formatCents(snap.commercial.aovCents, currency)
              }
            />
            <Stat label="Refunds" value={snap.commercial.refundOrders} />
            <Stat
              label="Refund amount"
              value={formatCents(snap.commercial.refundAmountCents, currency)}
            />
          </section>
          ...
          <section className="grid gap-3 lg:grid-cols-2">
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-950/60 p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm text-slate-300">
                ...
                <li>
                  Checkout created → paid: {formatPct(snap.health.checkoutCreatedToPaid)}
                </li>
                <li>
            ...
            {snap.attribution.byUtmSource.length === 0 ? (
              ...
              <ul className="mt-3 space-y-1 text-sm text-slate-300">
                {snap.attribution.byUtmSource.slice(0, 12).map((row) => (
                  <li key={row.source}>
                    {row.source}: {row.paidOrders} paid ·{" "}
                    {formatCents(row.revenueCents, currency)}
                  </li>
                ))}
              </ul>
