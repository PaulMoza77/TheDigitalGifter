...
export default function AdminFunnelPage() {
  ...
  const stats = useMemo(() => {
    ...
    const inOcc = (rows ?? []).filter(
      (r) => normalizeOccasion(r.occasion ?? "") === occ
    );
  ...
  async function toggleActive(r: TemplateDbRow, next: boolean) {
    ...
    setRows((p) =>
      p.map((x) => (x.id === r.id ? { ...x, isactive: next } : x))
    );
    ...
    if (error) {
      setRows((p) =>
        p.map((x) => (x.id === r.id ? { ...x, isactive: prev } : x))
      );
      toast.error(error.message);
    }
  }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Configurează ocaziile + stilurile (title + prompt) folosite în
            Funnel Style Select.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-12">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setOccasion(normalizeOccasion(v))}
                >
        ...
        <Card className="rounded-2xl border-slate-800 bg-slate-950 text-slate-50 lg:col-span-4">
          <CardHeader className="pb-3">
            ...
            <CardDescription className="text-slate-400">
              În FunnelStyleSelect se face query pe <b>templates</b> filtrat
              după <b>occasion</b> + <b>isactive</b>.
            </CardDescription>
          </CardHeader>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <div className="space-y-2 sm:col-span-2">
                ...
                <Input
                  value={form.ai_notes}
                  onChange={(e) =>
                    setForm((s) => ({ ...s, ai_notes: e.target.value }))
                  }
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setForm((s) => ({
                      ...
                      title,
                      style_id:
                        s.style_id || (title ? makeStyleIdFromTitle(title) : ""),
                    }));
                  }}
              ...
              <div className="space-y-2">
                ...
                <Input
                  value={form.style_id}
                  onChange={(e) =>
                    setForm((s) => ({ ...s, style_id: e.target.value }))
                  }
