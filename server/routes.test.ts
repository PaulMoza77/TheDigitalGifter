...
describe("TDG origin path classification", () => {
  it("keeps health, Apple Pay, and known APIs off the SPA fallback", () => {
    ...
    expect(classifyPath("/api/pet-v4/funnel-event").kind).toBe("api");
    expect(classifyPath("/api/pet-v4-funnel-event").kind).toBe("api");
    ...
    expect(classifyPath("/api/admin/social/youtube/callback").kind).toBe("api");
    expect(classifyPath("/api/admin/social/youtube/callback")).toEqual({
      kind: "api",
      module: "youtube-oauth-callback.ts",
    });
