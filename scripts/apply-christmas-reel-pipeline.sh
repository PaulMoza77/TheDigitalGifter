#!/usr/bin/env bash
# Apply Christmas reel publish pipeline migration to production Supabase.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
MIGRATION="supabase/migrations/20260926200000_christmas_reel_publish_pipeline.sql"
...
apply_via_management_api() {
  ...
  ' "$MIGRATION")"
  code="$(curl -sS -o /tmp/christmas-reel-pipeline-sql.json -w '%{http_code}' \
    -X POST "https://api.supabase.com/v1/projects/${PROJECT_REF}/database/query" \
    -H "Authorization: Bearer ${SUPABASE_ACCESS_TOKEN}" \
    -H "Content-Type: application/json" \
    -d "$payload" || true)"
  echo "Management API HTTP ${code}"
  head -c 500 /tmp/christmas-reel-pipeline-sql.json 2>/dev/null || true
  echo
