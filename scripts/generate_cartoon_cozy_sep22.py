#!/usr/bin/env python3
"""Generate 5 Higgsfield Kling 3.0 Pro I2V shorts (5s, sound=off) for cartoon-cozy stills.
...
Does not auto-retry QC-rejected creative failures.
...
DURATION = 5
BUDGET_USD = 1.45
MAX_PAID_GENERATIONS = 5

ROOT = Path(os.environ.get("TDG_ROOT") or "/workspace")
SOURCE = Path(os.environ.get("TDG_SOURCE") or (ROOT / "source" / "cartoon-cozy-sep22"))
MASTERS = Path(
    os.environ.get("TDG_MASTERS") or (ROOT / "public" / "assets" / "christmas" / "cartoon-cozy-sep22" / "masters")
)
POSTERS = Path(
    os.environ.get("TDG_POSTERS") or (ROOT / "public" / "assets" / "christmas" / "cartoon-cozy-sep22" / "posters")
)
QC = Path(os.environ.get("TDG_QC") or (ROOT / "generated" / "cartoon-cozy-sep22" / "qc"))
LOG = Path(
    os.environ.get("TDG_LOG")
    or (ROOT / "public" / "assets" / "christmas" / "cartoon-cozy-sep22" / "generation_manifest.json")
)
GENLOG = Path(os.environ.get("TDG_GENLOG") or (ROOT / "generated" / "cartoon-cozy-sep22" / "generation_log.json"))

NEGATIVE = (
    " Locked camera. Characters stay exactly where they are — no walking, no standing up, no exaggerated gestures. "
    "Do not morph faces, hands, bodies, clothing, or furniture. No extra limbs. No duplicated characters. "
    "Do not add or remove characters or objects. Do not redesign the scene. No melting objects. "
    "No random text, logos, captions, or watermarks. No audio. Subtle living-photograph motion only."
)

CLIPS = [
    {
        "id": "cozy_01_cartoon_cabin_mashup",
        "filename": "cozy_01_cartoon_cabin_mashup.mp4",
        "image": SOURCE / "01_cartoon_cabin_mashup_1080x1920.png",
        "source_still": "cartoon_cabin_mashup.jpg",
        "title": "Short · Cartoon cabin mashup",
        "library_id": "short-cozy-01-cartoon-cabin-mashup",
        "prompt": (
            "This exact cartoon cabin interior photograph quietly comes alive for five seconds. Locked camera. "
            "Preserve every character, face, body, outfit, and pose exactly: pink panther on the windowsill with mug, "
            "sponge in the hammock, duck in the beanbag, knitting grandmother, orange lobster on the sofa, scientist "
            "at the blueprint table, blonde man in sunglasses under a blanket, blue-haired woman reading, sleeping pink "
            "dog, sleeping brown monkey in the pet bed. "
            "Only four natural motions: rain streaks moving on the storm window with one brief lightning illumination "
            "of the mountains and lake; tiny steam rising from the mug; oil-lantern and candle flames flickering; "
            "sleeping dog and monkey with extremely subtle breathing. Everyone else stays almost still — maybe one tiny "
            "blink. Furniture, wood walls, books, and composition unchanged."
            + NEGATIVE
        ),
    },
    {
        "id": "cozy_02_family_christmas_boardgame",
        "filename": "cozy_02_family_christmas_boardgame.mp4",
        "image": SOURCE / "02_family_christmas_boardgame_1080x1920.png",
        "source_still": "family_christmas_boardgame.jpg",
        "title": "Short · Family Christmas board game",
        "library_id": "short-cozy-02-family-boardgame",
        "prompt": (
            "This exact cozy Christmas living-room photograph quietly comes alive for five seconds. Locked camera. "
            "Preserve the same family of four, golden retriever, sleeping grey kitten, Christmas tree, stone fireplace, "
            "stockings, wreath, board game, popcorn, mugs, plaid blankets, and snowy mountain village outside. "
            "Same faces, hair, sweaters, and seated positions. Nobody stands or walks. "
            "Only four natural motions: realistic fireplace flames with a soft changing warm glow; Christmas tree and "
            "garland lights twinkling gently; natural falling snow outside the windows; tiny steam from mugs and a "
            "barely-there blink or finger nudge on the board game from the girl. Dog stays lying down. Kitten stays asleep."
            + NEGATIVE
        ),
    },
    {
        "id": "cozy_03_tom_jerry_christmas_living_room",
        "filename": "cozy_03_tom_jerry_christmas_living_room.mp4",
        "image": SOURCE / "03_tom_jerry_christmas_living_room_1080x1920.png",
        "source_still": "tom_jerry_christmas_living_room.jpg",
        "title": "Short · Tom & Jerry Christmas living room",
        "library_id": "short-cozy-03-tom-jerry-living-room",
        "prompt": (
            "This exact Tom and Jerry Christmas living-room still quietly comes alive for five seconds. Locked camera. "
            "Preserve Tom on the left sofa with the red mug, Jerry standing on the coffee table holding cheese, Spike "
            "the bulldog on the right sofa, the small grey kitten with teddy under the blanket, Christmas tree, fireplace, "
            "stockings, board game, cookies, marshmallows, and snowy village windows. Same faces, bodies, and positions. "
            "Nobody walks. "
            "Only four natural motions: fireplace flames and warm glow; tree lights twinkling; falling snow outside; "
            "tiny steam from Tom's mug plus Jerry's tiny cheese wiggle and one small blink. Spike stays seated. Kitten stays still."
            + NEGATIVE
        ),
    },
    {
        "id": "cozy_04_tom_jerry_storm_sleeping",
        "filename": "cozy_04_tom_jerry_storm_sleeping.mp4",
        "image": SOURCE / "04_tom_jerry_storm_sleeping_1080x1920.png",
        "source_still": "tom_jerry_storm_sleeping.jpg",
        "title": "Short · Tom & Jerry storm sleeping cabin",
        "library_id": "short-cozy-04-tom-jerry-storm-sleep",
        "prompt": (
            "This exact rainy cabin den still quietly comes alive for five seconds. Locked camera. "
            "Preserve Tom sitting on the window frame with the steaming mug, Jerry smiling in the hammock with cheese, "
            "Spike asleep on the big plaid dog bed, black-and-white cat asleep, two grey kittens asleep with a teddy, "
            "yellow bird asleep, wood stove fire, lanterns, books, and the storm lake window. Same characters and poses. "
            "Nobody gets up. "
            "Only four natural motions: rain moving on the window with one subtle lightning flash lighting the mountains; "
            "steam rising from Tom's mug; wood-stove and lantern flame flicker; sleeping Spike, cats, kittens, and bird "
            "with extremely subtle breathing. Jerry may nibble the cheese a tiny bit. Furniture and sign text unchanged."
            + NEGATIVE
        ),
    },
    {
        "id": "cozy_05_tom_jerry_storm_bed",
        "filename": "cozy_05_tom_jerry_storm_bed.mp4",
        "image": SOURCE / "05_tom_jerry_storm_bed_1080x1920.png",
        "source_still": "tom_jerry_storm_bed.jpg",
        "title": "Short · Tom & Jerry stormy bed",
        "library_id": "short-cozy-05-tom-jerry-storm-bed",
        "prompt": (
            "This exact Tom and Jerry cabin-bed still quietly comes alive for five seconds. Locked camera. "
            "Preserve Tom sleeping on the left pillows, Jerry sitting in the blankets holding cheese, empty hammock, "
            "wood stove fire, lanterns, stacked books, mug, cookies, open comic, plaid pillows, and stormy lake window. "
            "Same faces, bodies, clothing, and layout. "
            "Only four natural motions: rain streaks on the window with one brief lightning illumination; lantern and "
            "stove flames flickering; Tom's extremely subtle sleeping breath; Jerry a tiny nibble or blink while holding "
            "cheese. No one leaves the bed. Book spines and mug lettering stay readable and unchanged."
            + NEGATIVE
        ),
    },
]
...
def hf_request(method: str, path_or_url: str, data: dict | None = None, timeout: int = 180) -> dict:
    ...
    headers = {
        ...
        "User-Agent": "tdg-cartoon-cozy-sep22/higgsfield",
...
def download(url: str, dest: Path) -> None:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "tdg-cartoon-cozy-sep22/higgsfield"})
    ...
    for path in (LOG, GENLOG):
        if path.exists():
            try:
                ...
                if isinstance(data, dict) and data.get("status") != "ESTIMATE_ONLY":
                    return data
...
def main() -> int:
    ...
    for clip in CLIPS:
        if existing_clip(log, clip["id"]) or pending_job_id(log, clip["id"]):
            continue
        if clip["id"] not in uploads:
            continue
        ...
        projected = float(sum(c.get("generation_cost_usd") or 0 for c in log["clips"])) + float(
            est_by_id[clip["id"]]["usd"]
        )
        ...
        print(f"  job={request_id} status={submitted.get('status')} paid_submits={log['paid_submits']}", flush=True)
        time.sleep(0.3)
    ...
    for clip in CLIPS:
        ...
        if not pending:
            raise RuntimeError(f"missing pending job for {clip['id']}")
        try:
            entry = submit_and_download(clip, est_by_id[clip["id"]], pending_job=pending)
    ...
    write_logs(log)
    print(f"COMPLETE clips={len(log['clips'])}/5 actual_cost=${log['actual_cost_usd']:.4f}", flush=True)
    return 0
