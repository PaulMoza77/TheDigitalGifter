#!/usr/bin/env python3
"""Generate 5 Higgsfield Kling 3.0 Pro I2V clips for the global Christmas journey reel.
...
SOURCE = Path(os.environ.get("TDG_SOURCE") or (ROOT / "source" / "global-christmas"))
MASTERS = Path(
    os.environ.get("TDG_MASTERS") or (ROOT / "public" / "assets" / "christmas" / "global-reel" / "masters")
)
POSTERS = Path(
    os.environ.get("TDG_POSTERS") or (ROOT / "public" / "assets" / "christmas" / "global-reel" / "posters")
)
QC = Path(os.environ.get("TDG_QC") or (ROOT / "generated" / "global-christmas" / "qc"))
LOG = Path(
    os.environ.get("TDG_LOG")
    or (ROOT / "public" / "assets" / "christmas" / "global-reel" / "generation_manifest.json")
)
GENLOG = Path(os.environ.get("TDG_GENLOG") or (ROOT / "generated" / "global-christmas" / "generation_log.json"))

ANTI_AI = (
    " Photoreal live-action cinema camera, not a Ken Burns still. Different motion speeds at different depths: "
    "foreground, midground, and background. Do not morph architecture. Do not bend buildings, windows, columns, "
    "roofs, trees, cars, or people. Do not change signs or generate nonsense text. Do not duplicate pedestrians. "
    "Do not deform faces. Do not make snow move unnaturally. No hyperlapse. No excessive camera travel. "
    "No pulsing lights. No floating objects. No new major objects. Stabilized cinematic camera. No generated audio."
)

CLIPS = [
    {
        "id": "global_01_lapland",
        "filename": "global_01_lapland.mp4",
        "image": SOURCE / "01_lapland_aurora_1080x1920.png",
        "source_still": "01_lapland_aurora.jpg",
        "title": "Lapland aurora glass igloos",
        "prompt": (
            "Photoreal cinematic 9:16 Lapland Christmas night. This exact photograph was filmed with a cinema camera "
            "for five seconds. Preserve the snow-laden pine, wooden deck, fur lounge chairs, fire pit, lanterns, "
            "glass igloo lodges, lakeside village cabins, frozen lake, islands, and green aurora exactly as in the "
            "first frame. Slow expensive cinematic camera push toward the glass igloos and lake. Natural falling snow "
            "with real gravity. Aurora slowly drifts and folds across the sky. Subtle chimney smoke and steam. "
            "Fire in the pit flickers naturally. Christmas lights shimmer gently, never pulse. Realistic reflections "
            "on snow, glass, and water. Tiny atmospheric haze. Architecture and trees stay rigid."
        )
        + ANTI_AI,
    },
    {
        ...
        "image": SOURCE / "02_alps_infinity_pool_1080x1920.png",
        "source_still": "02_alps_infinity_pool.jpg",
        "title": "Alps infinity pool hotel",
        "prompt": (
            "Photoreal cinematic 9:16 luxury Alpine hotel commercial. This exact photograph was filmed with a cinema "
            "camera for five seconds. Preserve the stone chalet, BELLEVUE ALPINE RESORT towels, infinity pool, "
            "Matterhorn mountain, lake, lounge chairs, lanterns, Christmas trees, fireplace terrace, and sunset sky "
            "exactly as in the first frame. Extremely slow camera glide toward the infinity pool and mountains. Pool "
            "water moves naturally with steam rising from heated water. Fireplace flames move naturally. Subtle "
            "snowfall. Christmas lights glow gently. Clouds drift almost imperceptibly. Reflections travel across "
            "the water. The mountain and hotel remain structurally perfectly stable. Do not change towel lettering."
        )
        + ANTI_AI,
    },
    {
        ...
        "image": SOURCE / "03_humble_christmas_home_1080x1920.png",
        "source_still": "03_humble_christmas_home.jpg",
        "title": "Humble Christmas home",
        "prompt": (
            "Photoreal cinematic 9:16 coming-home-for-Christmas film. Warm, emotional, human, not luxury. This exact "
            "photograph was filmed with a cinema camera for five seconds. Preserve the small white cottage, green "
            "door, porch wreath, snowman, stone path, lanterns, red bows, string lights, and snowy trees exactly as "
            "in the first frame. Slow approach toward the house as if walking home. Gentle snowfall. Chimney smoke "
            "drifts naturally. Christmas lights flicker only as real bulbs would. Warm interior light. Subtle fire "
            "and lamp movement visible through windows. Tiny winter-wind movement in tree branches. Nostalgic, safe, "
            "real neighborhood Christmas evening. No new people. No luxury cars."
        )
        + ANTI_AI,
    },
    {
        ...
        "image": SOURCE / "04_plaza_new_york_1080x1920.png",
        "source_still": "04_plaza_new_york.jpg",
        "title": "The Plaza New York",
        "prompt": (
            "Photoreal cinematic 9:16 Manhattan Christmas night. Genuine street footage of The Plaza Hotel. This exact "
            "photograph was filmed with a cinema camera for five seconds. Preserve The Plaza architecture, THE PLAZA "
            "NEW YORK canopy lettering, wreaths, wet pavement, black sedan, white horse and carriage, doormen, "
            "existing pedestrians, Christmas trees, and skyscrapers exactly as in the first frame. Slow cinematic "
            "camera toward the entrance. Existing pedestrians take a few natural steps only — do not add crowds, do "
            "not duplicate people, do not distort faces. Subtle car creep of a few inches, never warp the sedan. "
            "Horse and carriage make tiny realistic movement. Falling snow. Reflections move across wet pavement. "
            "Subtle street steam. Distant city lights. Do not change signage or architecture."
        )
        + ANTI_AI,
    },
    {
        "id": "global_05_white_house",
        "filename": "global_05_white_house.mp4",
        "image": SOURCE / "05_white_house_1080x1920.png",
        "source_still": "05_white_house.jpg",
        "title": "White House Christmas",
        "prompt": (
            "Photoreal cinematic 9:16 Washington D.C. Christmas dusk. This exact photograph was filmed with a cinema "
            "camera for five seconds. Preserve the White House north facade, columns, windows, American flag, "
            "fountain, iron fence, wreaths, Christmas tree, THE WHITE HOUSE WASHINGTON plaque, lanterns, and wet "
            "pavement exactly as in the first frame. Slow stabilized cinematic push toward the White House. Gentle "
            "snowfall. American flag moves naturally in light wind. Fountain water moves naturally. Christmas lights "
            "shimmer subtly. Tree lights glow. Wet pavement reflections. Subtle tree branch movement. Architecture, "
            "signage, windows, columns, flag proportions, fence, and plaque stay completely stable. Elegant final "
            "monumental Christmas image. No people added."
        )
        + ANTI_AI,
    },
]
...
CONSERVATIVE_SUFFIX = (
    " CONSERVATIVE MOTION ONLY. Minimal camera travel. Almost locked-off tripod with a tiny push-in. "
    "Environment motion only: sparse snow, light shimmer, water, steam, flag, reflections. Zero architecture change. "
    "People and cars almost still."
)
...
def hf_request(method: str, path_or_url: str, data: dict | None = None, timeout: int = 180) -> dict:
    ...
    headers = {
        ...
        "User-Agent": "tdg-global-christmas-reel/higgsfield",
...
def download(url: str, dest: Path) -> None:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "tdg-global-christmas-reel/higgsfield"})
...
def verify_auth() -> None:
    ...
    print("MODE: image-to-video  DURATION: 5s  SOUND: off  TARGET: 1080x1920", flush=True)
...
def main() -> int:
    ...
    print("INPUT IMAGES", flush=True)
    for clip in clips:
        if not clip["image"].exists():
            raise SystemExit(f"missing still {clip['image']}")
        info = probe_summary(clip["image"]) if clip["image"].suffix.lower() in {".mp4"} else None
        print(f"  {clip['id']}: {clip['image'].name}  {clip['image'].stat().st_size} bytes", flush=True)
    ...
    print("DURATION: 5s  SOUND: off  MODE: image-to-video")
    for clip in clips:
