#!/usr/bin/env bash
# Apply Publisher V1 migration to production Supabase.
# Target project MUST be kjlsocejpmnzhhduyumy (TheDigitalGifter).
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"
MIGRATION="supabase/migrations/20260923120000_publisher_v1.sql"
...
apply_via_management_api() {
  ...
  ' "$MIGRATION")"
  code="$(curl -sS -o /tmp/publisher-v1-sql.json -w '%{http_code}' \
    -X POST "https://api.supabase.com/v1/projects/${PROJECT_REF}/database/query" \
    -H "Authorization: Bearer ${SUPABASE_ACCESS_TOKEN}" \
    -H "Content-Type: application/json" \
    -d "$payload" || true)"
  echo "Management API HTTP $code"
  head -c 500 /tmp/publisher-v1-sql.json 2>/dev/null || true
  echo
...
echo "Publisher V1 migration applied to $PROJECT_REF."
